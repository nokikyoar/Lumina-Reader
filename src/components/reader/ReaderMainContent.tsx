  ...
  return (
    <div className="flex-1 flex flex-col h-full relative min-w-0">
      ...
      <div className="flex-1 relative overflow-hidden" id="pdf-container">
        ...
        {p.book.type === 'web' && <div className="h-full w-full overflow-auto p-4 md:p-6 lg:p-8"><motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24, ease: 'easeOut' }} className={cn('mx-auto w-full rounded-2xl border p-3 md:p-4 space-y-3', p.styles.toolbarGroupBg, p.styles.sidebarBorder)}><div className="flex flex-col md:flex-row gap-2"><input value={p.webUrl} onChange={(e) => p.setWebUrl(e.target.value)} placeholder="Enter website URL, e.g. https://example.com" className={cn('flex-1 rounded-lg px-3 py-2 text-sm outline-none border transition-all duration-200 focus:ring-4 focus:ring-[#0052FF]/10', p.styles.bg, p.styles.text, p.styles.sidebarBorder)} /><button onClick={() => { const normalized = p.normalizeWebInputUrl(p.webUrl); if (!normalized) return; p.setWebUrl(normalized); p.setWebFrameStatus('loading'); p.setWebFrameHint('Recommended production approach: enable browser-extension bridge (content script + postMessage) for in-page selection, highlights, notes, and AI quotes.'); }} className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#0052FF] to-[#4D7CFF] text-white text-sm shadow-[0_12px_24px_-14px_rgba(0,82,255,0.8)] transition-all duration-200 hover:-translate-y-0.5 hover:opacity-95">Load URL</button></div><div className={cn('text-xs', p.webBridgeConnected ? 'text-emerald-600' : 'text-amber-600')}>{p.webBridgeConnected ? `Bridge status: connected${p.webBridgeVersion ? ` (v${p.webBridgeVersion})` : ''}` : 'Bridge status: waiting for extension bridge handshake'}</div>{p.webBridgeLastAction && <div className="text-xs text-emerald-700">Last bridge action: {p.webBridgeLastAction}</div>}{p.webBridgeLastError && <div className="text-xs text-red-600">Last bridge error: {p.webBridgeLastError}</div>}{p.webFrameHint && <div className="text-xs text-amber-600">{p.webFrameHint}</div>}</motion.div><motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24, delay: 0.04, ease: 'easeOut' }} className="mt-4 h-[calc(100%-7.5rem)] min-h-[420px] rounded-2xl overflow-hidden border"><div className="relative w-full h-full"><iframe ref={p.webIframeRef} src={p.normalizeWebInputUrl(p.webUrl)} className="w-full h-full bg-white" sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox" referrerPolicy="no-referrer" title={p.book.title} onLoad={() => { p.setWebFrameStatus('loaded'); p.setWebFrameHint('Embed loaded. For in-page highlights/AI quotes, enable the browser-extension bridge.'); }} onError={() => { p.setWebFrameStatus('error'); p.setWebFrameHint('Embed failed: target site may block iframe embedding (X-Frame-Options/CSP).'); }} />{p.webFrameStatus === 'loading' && <div className="absolute inset-0 grid place-items-center text-sm bg-white/80">Trying to embed website...</div>}</div></motion.div></div>}
